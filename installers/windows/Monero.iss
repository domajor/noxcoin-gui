; NoxCoin Fluorine Fermi GUI Wallet Installer for Windows
; Copyright (c) 2017-2024, The NoxCoin Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\noxcoin-wallet-gui.exe")

[Setup]
AppName=NoxCoin GUI Wallet
...
DefaultDirName={commonpf}\NoxCoin GUI Wallet
DefaultGroupName=NoxCoin GUI Wallet
...
LicenseFile=LICENSE
AppPublisher=The NoxCoin Developer Community
AppPublisherURL=https://getnoxcoin.org
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the NoxCoin files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to NoxCoin.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; NoxCoin GUI wallet exe and guide
Source: "bin\noxcoin-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\noxcoin-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; NoxCoin CLI wallet
Source: "bin\extras\noxcoin-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; NoxCoin wallet RPC interface implementation
Source: "bin\extras\noxcoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; NoxCoin daemon
Source: "bin\noxcoind.exe"; DestDir: "{app}"; Flags: ignoreversion

; NoxCoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "noxcoin-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; NoxCoin blockchain utilities
Source: "bin\extras\noxcoin-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\noxcoin-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\noxcoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitnoxcoin');
  s := 'The default folder to store the NoxCoin blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\noxcoin-project\noxcoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" NoxCoin default location
...
begin
  Result := BlockChainDir('') + '\bitnoxcoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "noxcoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the NoxCoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\noxcoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\noxcoin-daemon.bat'), s, false); 
  end;
end;
...
Name: "{group}\Utilities\NoxCoin Daemon"; Filename: "{app}\noxcoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\noxcoin-wallet-cli.exe"; WorkingDir: "{userdocs}\NoxCoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\NoxCoin\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\noxcoin-wallet-gui\noxcoin-wallet-gui.log"""
...
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM noxcoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\noxcoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\noxcoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\noxcoin-project\noxcoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\noxcoin-project\noxcoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "noxcoin:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "noxcoin://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "noxcoin"; ValueType: "string"; ValueData: "URL:NoxCoin Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "noxcoin"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "noxcoin\DefaultIcon"; ValueType: "string"; ValueData: "{app}\noxcoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "noxcoin\shell\open\command"; ValueType: "string"; ValueData: """{app}\noxcoin-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "noxcoinseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "noxcoinseed://a.b.c.d:port"
Root: HKCR; Subkey: "noxcoinseed"; ValueType: "string"; ValueData: "URL:NoxCoin Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "noxcoinseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "noxcoinseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\noxcoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "noxcoinseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\noxcoin-wallet-gui.exe"" ""%1"""
