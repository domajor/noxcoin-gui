...
overriden with `-Dnoxcoin_CRYPTO_LIBRARY=amd64-51-30k` or by setting the
...
can be overriden with `-Dnoxcoin_CRYPTO_LIBRARY=amd64-51-30k` at the first
cmake step. Using `-Dnoxcoin_CRYPTO_LIBRARY=invalid` will generate an error
