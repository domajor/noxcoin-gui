...
gsigs = 'https://github.com/noxcoin-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('noxcoin'):
        # Remove the potentially stale noxcoin dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'noxcoin', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'noxcoin'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/noxcoin/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'noxcoin='+args.commit, '--url', 'noxcoin='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/noxcoin-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/noxcoin/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/noxcoin/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/noxcoin-project/noxcoin', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/noxcoin', exist_ok=True)
    os.chdir('builder/inputs/noxcoin')
    if args.pull:
