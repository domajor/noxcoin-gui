...
bool is_unsafe(unsigned char compare_char)
{
    ...
    for (int ichar_pos = 0; punsave[ichar_pos] != 0; ++ichar_pos)
...
std::string dec_to_hex(char num, int radix)
{
    int temp = 0;
    std::string csTmp;
    int num_char = static_cast<int>(num);
    if (num_char < 0)
        num_char += 256;
    ...
    {
        temp = num_char % radix;
        num_char = static_cast<int>(std::floor(static_cast<float>(num_char) / static_cast<float>(radix)));
...
{
    const char* ptr = static_cast<const char*>(memchr(s, c, 16));
    return ptr ? ptr - s : -1;
}
...
std::string hex_to_dec_2bytes(const char* s)
{
    ...
    return std::string(1, static_cast<char>(i0 * 16 | i1));
}
...
//----------------------------------------------------------------------------------------------------
bool abstract_http_client::set_server(const std::string& address, boost::optional<login> user, ssl_options_t ssl_options)
{
...
//----------------------------------------------------------------------------------------------------
bool abstract_http_client::set_proxy(const std::string& /*address*/)
{
    return false;
}
